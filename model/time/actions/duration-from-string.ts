...
export default function(durationString?: string): Duration | undefined
{
  ...
  const unit = durationString.trim().match(/[A-Za-z][A-Za-z\/\d]+$/g)?.[0] as TimeUnit | undefined
  let amount: number
  ...
  {
    throw new InvalidInputError(`Could not parse the duration string: ${durationString}`)
  }
  return { amount, unit }
}
