...
export default function(weightString?: string): Weight | undefined
{
  ...
  const unit = weightString?.trim().match(/[A-Za-z][A-Za-z\/\d]+$/g)?.[0] as WeightUnit | undefined
  let amount: number
  ...
  {
    throw new InvalidInputError(`Could not parse the weight string: ${weightString}`)
  }
  return { amount, unit }
}
