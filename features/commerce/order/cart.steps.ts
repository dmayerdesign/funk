...
defineFeature(feature, function (example) {
  ...
  describe("A user must always have a shopping cart.", function () {
    example("When Newt is created, a cart is created for Newt.", function ({ given, when, then }) {
    ...
    example("When Annie submits an order, a new cart is created for Annie.", async function ({ given, when, then }) {
      given(/a customer named ([\w\s]+)$/, async function () {
        const customerData = await constructGivenACustomer(
          customerHandleCreate
        )("Annie")
  ...
  describe("A user may only add in-stock products to their cart.", function () {
    example("Sally can add the in-stock SKU Rollerblades to their cart.", function ({ given, when, then }) {
      ...
      when(/([\w\s]+) tries to add ([\w\s]+) to their cart$/, async function () {
  ...
  describe('A user must go through a "checkout" flow before submitting an Order.', function () {
    ...
    function behaviorGivenThatTheCartContainsInStockSkus(given: DefineStepFunction) {
      given(/that ([\w\s]+)'s cart contains in-stock SKUs$/, async function () {
        await givenThatTheCartContainsInStockSkus({ theCart: cart })
      })
    }

    example('Chuck begins the "checkout" flow.', function ({ given, when, then }) {
      ...
      then(/the status of ([\w\s]+)'s cart changes to "Cart Checkout"$/, async function () {
        const [theUpdatedCart] = await listOrdersForUser(person.id)
        expect(theUpdatedCart.status).toBe(Status.CART_CHECKOUT)
      })
    })

    example('Chuck completes the "checkout" flow with errors.', function ({ given, when, then }) {
      given(/a customer named ([\w\s]+)/, async function (customerName: string) {
        const customerData = await constructGivenACustomer(
          customerHandleCreate
        )(customerName)
      ...
      given(/that ([\w\s]+) has provided their payment information$/, async function () {
      ...
      given(/that ([\w\s]+) has provided their shipping address and chosen a shipping rate$/, async function () {
      ...
      then(/the status of ([\w\s]+)'s order changes to "Payment Pending"/, async function () {
        const [theUpdatedCart] = await listOrdersForUser(person.id)
  ...
  describe(
    ...
    function () {
      ...
      example("Sam and Cam each put one pair of shoes in their cart, and Sam begins checkout.", function ({ given, when, then }) {

        let coolShoes: MarshalledSku | undefined

        given(/2 customers named (\w+) and (\w+)$/, async function (customer1Name: string, customer2Name: string) {
        ...
        given(/an in-stock SKU named ([\w\s]+) with 1 left in inventory$/, async function () {
          await setById(
            SKUS,
            "cool-shoes",
        ...
        given(/that Sam and Cam each put ([\w\s]+) into their carts$/, async function () {
        ...
        then(/Cam is no longer able to purchase ([\w\s]+)$/, async function () {
          await expect(setStatusToCheckout(cart2.id)).rejects.toThrow(
            new RegExp(SKUS_OUT_OF_STOCK_ERROR, "g")
          )
        })

        then(/the SKU's stock quantity appears to Cam as zero$/, async function () {
          coolShoes = await getById<MarshalledSku>(SKUS, "cool-shoes")
