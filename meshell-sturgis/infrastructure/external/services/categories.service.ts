...
export class CategoriesService {
  ...
  getOne(categorySlug: string): Observable<any> {
    ...
    return this.http.get<any[]>(`${this.cs.config.rootPath}/${this.endpoint}`, { params })
    ...
    if (exclude && exclude.length < 4) {
      return this.getIdsOf([...exclude]).pipe(
        switchMap(ids => {
          ...
          return this.http.get<any[]>(`${this.cs.config.rootPath}/${this.endpoint + queryString}`);
        })
      )
    }
  }
  ...
  getById(categoryId: number): Observable<any> {
    return this.http.get<any>(`${this.cs.config.rootPath}/${this.endpoint}/${categoryId}`)
  }
  ...
  getIdsOf(categorySlugs: string[]): Observable<number[]> {
    ...
    return this.http.get<any[]>(`${this.cs.config.rootPath}/${this.endpoint}?embed=true${queryString}`)
  ...
  getAllButHome(): Observable<any[]> {
    ...
    return this.getIdOf('home').pipe(switchMap(id => {
      params = params.append('exclude[]', id.toString());
      return this.http.get<any[]>(`${this.cs.config.rootPath}/${this.endpoint}`, { params });
    }));
  }
