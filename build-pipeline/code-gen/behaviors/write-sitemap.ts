...
export default async function(baseUrl: string): Promise<void>
{
  ...
  function buildSitemapUrls(atlas: Atlas, _baseUrl = baseUrl, isRoot = true): SitemapUrl[]
  {
    ...
    return Object.keys(atlas)
      ...
      .reduce(
        (acc, key) =>
        {
          ...
          const url = new URL([ ...(basePaths ?? []), key ].join("/"), protocol + domain).toString()
...
async function getRootAtlas(): Promise<Atlas>
{
  ...
  const atlasFilenames = recursiveReaddir(uiDirname)
    .filter((filename) => filename.match(/atlas\.ts$/g))
