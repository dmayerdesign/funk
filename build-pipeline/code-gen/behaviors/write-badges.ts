...
const readmeIsWritableForBadges = readmeAsString.indexOf(readmeBadgeStartComment) > -1
  && readmeAsString.indexOf(readmeBadgeEndComment) > -1

const pathToUiCoverageReport = resolve(__dirname, "../../../", "coverage/ui/lcov-report/index.html")
const pathToApiCoverageReport = resolve(__dirname, "../../../", "coverage/api/lcov-report/index.html")
...
if (existsSync(pathToUiCoverageReport) && existsSync(pathToApiCoverageReport))
{
  ...
  const { linesCovered: uiLinesCovered, linesTotal: uiLinesTotal } = Array
    .from(uiWindow.document.querySelectorAll<HTMLTableCellElement>(
    ...
    .reduce((report, [ dividend, divisor ]) => ({
      linesCovered: report.linesCovered + dividend,
      linesTotal: report.linesTotal + divisor,
    }), { linesCovered: 0, linesTotal: 0 })
  const { linesCovered: apiLinesCovered, linesTotal: apiLinesTotal } = Array
    .from(apiWindow.document.querySelectorAll<HTMLTableCellElement>(
      ".coverage-summary tbody td:last-child"
    ))
    .map((node) => node.innerHTML?.trim())
    .filter((x) => !!x)
  ...
  const readmeSplit = readmeAsString.split(
    new RegExp(
      `${escapeRegExp(readmeBadgeStartComment)}|${escapeRegExp(readmeBadgeEndComment)}`
    )
  )
  ...
  if (readmeIsWritableForBadges)
  {
    ...
    const apiCoveragePercent = Math.floor(apiLinesCovered / apiLinesTotal * 100);
    const uiCoverageColor =
      uiCoveragePercent >= 90 ? "brightgreen"
      : uiCoveragePercent >= 80 ? "green"
      : uiCoveragePercent >= 70 ? "yellowgreen"
      : uiCoveragePercent >= 60 ? "yellow"
      : uiCoveragePercent >= 50 ? "orange"
      : "red"
    const apiCoverageColor =
      apiCoveragePercent >= 90 ? "brightgreen"
      : apiCoveragePercent >= 80 ? "green"
      : apiCoveragePercent >= 70 ? "yellowgreen"
      : apiCoveragePercent >= 60 ? "yellow"
      : apiCoveragePercent >= 50 ? "orange"
      : "red"
  ...
  {
    console.warn("Could not write README badges. Make sure README.md has the following comments:"
      + readmeBadgeStartComment + " and " + readmeBadgeEndComment)
  }
}
